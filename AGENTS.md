...
## Local Configuration (Fork Workflow)
...
**Before PR or merge**
- Rebase on latest upstream: `git fetch upstream` then `git rebase upstream/main`
- Ensure relevant tests or checks are run (note in PR if skipped).
...
- Typing: avoid `Any` and `type: ignore`; prefer simple, explicit types.
...
- Example:
- `claude --print "Please review this feature branch after checking AGENTS.md and CONTRIBUTING.md. <brief description of the feature and anything useful to point out about the code or the state of the branch>"`
...
- Reviews can take time; allow at least 1-2 minutes for the agent to respond.
- Fix substantive issues and continue the existing review session with `claude -c`. Use judgment on minor nits.
